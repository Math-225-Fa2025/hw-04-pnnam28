...
```{r lowest-unemp, eval=TRUE, echo=TRUE}
...
```{r lowest-unemp-select, eval=TRUE, echo=TRUE}
college_recent_grads %>%
  arrange(unemployment_rate) %>%
...
```{r percent_grads, eval=TRUE, echo=TRUE}
college_recent_grads %>%
  arrange(unemployment_rate) %>%
  select(rank, major, unemployment_rate) %>%
...
```{r recent_grads, eval=TRUE, echo=TRUE}
college_recent_grads %>%
...
```{r top3_recent_grads, eval=TRUE, echo=TRUE}
college_recent_grads %>%
...
```{r plot_median, eval=TRUE, echo=TRUE, figheight=2}
...
```{r plot_median_1000bins, eval=TRUE, echo=TRUE, figheight=2}
ggplot(data = college_recent_grads, mapping = aes(x = median)) +
  geom_histogram(binwidth = 1000)
```

```{r plot_median_5000bins, eval=TRUE, echo=TRUE, figheight=2}
ggplot(data = college_recent_grads, mapping = aes(x = median)) +
  geom_histogram(binwidth = 5000)
```

  ANSWERS: From 2 plots above, the 1000 binwidth give better observation from the range 20,000 to 40,000 whereas the 5000 will combine these 2 columns so there will be more difficulty for further analysis.

We can also calculate summary statistics for this distribution using the `summarise` function:

```{r summaryData, eval=TRUE, echo=TRUE}
college_recent_grads %>%
...
4.  Based on the shape of the histogram you created in the previous exercise, determine which of these summary statistics is useful for describing the distribution.
  ...
  ANSWER: the histogram above showed that the majority of median wage is 35,000,
whereas the median of the plot will be 36,000. The shape of the plot represented a downtrend of the job market that a not many company offered more than 35,000 after graduate. Beside from the previous point, a maximum a career paid is 110,000, on the other hand, the minimum would be 22,000. However, the spread of the plot is also not even, there was a massive gap between 76,000 to 110,000 which showed that there is no company offered more than 76,000 after graduate.
...
```{r plot_median_1000_fCatagories, eval=TRUE, echo=TRUE, figheight=2}
ggplot(data = college_recent_grads, mapping = aes(x = median)) +
  geom_histogram(binwidth = 1000)+
...
```{r highest_medianPerMajor,eval=TRUE,echo=TRUE}
college_recent_grads %>%
  group_by(major_category) %>%
  summarise(highest_med = max(median)) %>%
  arrange(desc(highest_med))
```
...
```{r least_popular,eval=TRUE,echo=TRUE}
college_recent_grads %>%
...
```{r STEM_highest_earning,echo=TRUE,eval=TRUE}
college_recent_grads %>%
  filter(
    major_type == "stem",
    median <= 36000
...
```{r plot_median_vs_women_vs_inSTEM, echo=TRUE,eval=TRUE, error=TRUE, message=TRUE,figheight=2}
college_recent_grads %>%
  ggplot(aes(x= median, y= sharewomen, color= major_type))+
  geom_point()+
  theme_minimal()+
  labs(
    title = "Comparing Between Median Earning With The Proportion Of Women",
    subtitle = "Catagorized by STEM major",
    x = "Median",
...
```
  ...
  ANSWER: From the plot, women in STEM got higher earnings compare with the other women who is not working in STEM job. Surprisingly, the highest earning made from about 12.5% are women in STEM, whereas there are more than 50% are women who  not working in STEM making lower than average earning after gradation.  
...
```{r lowWage_CollegeJobs, echo=TRUE,eval=TRUE, error=TRUE, message=TRUE, figheight=2}
college_recent_grads %>%
  ggplot(aes(x = college_jobs, y = low_wage_jobs, color = major_category)) +
  geom_point(alpha = 0.6) +
  scale_x_log10() + scale_y_log10() +
  labs(
    title = "College vs Low-Wage Jobs by Major",
    x = "Jobs Requiring College Degree",
    y = "Low-Wage Jobs",
    color = "Category"
  ) +
  theme_minimal()
```
  ...
  ANSWER: From the plot, we can see that there are alot of jobs require a degree with a high wage, which graduates major in Health, Physic&Science, Computer and Mathematics. 
